...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow){
	...
	mw.SetWindowStyle(WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_VSCROLL).WindowPos(0, 0).SetExStyle(0);
	//mw.Borderless();
	
	mw2.WClassName(L"my2").Register();
	mw2.SetWindowStyle(WS_CHILD);
	mw.Child(mw2, (HMENU)0);
	
	mw3.WClassName(L"my2");
	mw3.SetWindowStyle(WS_CHILD);
	mw.Child(mw3, (HMENU)0);
	
	
	//bw2.WClassName(L"BUTTON");
	//bw2.SetExStyle(0).WindowStyle(WS_VISIBLE).WindowPos(0, 0);
	/*
	bw2.Layout([](HDC hdc){
	...
	*/
	...
	//bw3.SetExStyle(0).WindowStyle(WS_VISIBLE).WindowPos(100, 0);
	//mw2.Child(bw3, (HMENU)ID_BUTTON3);
	
	
	//bw4.WClassName(L"BUTTON");
	//bw4.SetExStyle(0).WindowStyle(WS_VISIBLE ).WindowPos(50, 50);
